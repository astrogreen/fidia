(function(angular){
    ...
    app.controller('Ctrl2', ['$scope', '$http',  function ($scopeschemabrowser, $http) {
        /* -- INITIALIZATION -- */
        //
        // I/O CATALOGUES (ng-repeat loops over this to create select elements)
        $scopeschemabrowser.inputSchema = [];
        $scopeschemabrowser.outputSchema = [];
        ...
        function fBuildSchema(data) {
            ...
            $scopeschemabrowser.inputSchema.push([]);
            $scopeschemabrowser.inputSchema[0] = data;
        };
        ...
        $scopeschemabrowser.fDropClick = function (key) {

            var restricted_arr = ['ticked', 'columns', 'contact'];

            // angular.forEach($scopeschemabrowser.outputSchema, function (value, key) {
            // current dropdown (row) == key
            var key = key;
            angular.forEach($scopeschemabrowser.outputSchema[key], function (valuea, keya) {
                // keya == 0 (each in arr)
                $('#schema-element-' + key + '  .information dl').html('')
                $('#schema-element-' + key + '  .description').html('')
                var descriptions = [];
                angular.forEach($scopeschemabrowser.outputSchema[key][keya], function (valueb, keyb) {
                    ...
                    if ((typeof valueb == 'object') && (valueb !== null)) {
                        ...
                        var k = Object.keys($scopeschemabrowser.inputSchema).length - 1;
                        $scopeschemabrowser.inputSchema[k] = valueb;
                    }
                    // Sort alphabetically...
